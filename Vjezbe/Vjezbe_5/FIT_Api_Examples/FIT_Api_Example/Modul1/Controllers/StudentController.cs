...
namespace FIT_Api_Example.Modul2.Controllers
{
    ...
    public class StudentController : ControllerBase
    {
        ...
        [HttpPost]
        public ActionResult UpdateOrAdd([FromBody] StudentUpdateVM x)
        {
            Student? student;
            if (x.id == 0)//Radi se o novom studentu koji ne postoji;
            {
                student=new Student();
            ...
            else//Student se edituje
            {
                student = _dbContext.Student.FirstOrDefault(s=>s.id==x.id);
                if(student ==null)
                    return BadRequest("Nema studenta sa tim ID!");
            }
            ...
            student.opstina_rodjenja = _dbContext.Opstina.Where(o => o.id == x.opstina_rodjenja_id).ToList()[0];
